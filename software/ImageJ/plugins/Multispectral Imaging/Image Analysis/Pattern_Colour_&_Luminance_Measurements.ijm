...
settingsFilePath = getDirectory("plugins") + "Multispectral Imaging/analysisSettings.txt";
...
Dialog.create("Pattern & Luminance Measurement");
	...
	Dialog.addMessage("Combining ROIs:");
	Dialog.addString("Prefix (separate with a comma)", defaultSettings[12]);
	Dialog.addMessage("Any ROIs starting with the codes you enter will be\ncombined and measured as a single area. For\nexample, \"t,egg\" will combine \'t\'s with \'t\'s and \'egg\'s\nwith \'egg\'s");
...
/*
// Actually this is annoying if the first image doesn't have the same number of regions as the subsequent ones
if(startSize != 0 && endSize != 0 && outputPatternSpectrum ==1)
	for(i=0; i<nSelections; i++) // make new columns for pattern & luminance at the start of the results sheet
		setResult(selNames[i] + "_patternHist", resultsRow, 0);

if(lumBands != 0)
	for(i=0; i<nSelections; i++)
		setResult(selNames[i] + "_lumHist", resultsRow, 0);
*/
...
// RESCALE IMAGE AND ROIs


if(scaleValue != 1){

	// SCALE ROIs BEFORE IMAGE - otherwise things off-the-scale are moved

	for(i=0; i<nSelections; i++){ // get names, resize & add
		roiManager("select", i);
		selNames[i] = getInfo("selection.name");
		//getBoundingRect(selectionX, selectionY, selectionW, selectionH);
		scaleString = "x="+ scaleValue + " y=" + scaleValue;
		run("Scale... ", scaleString);
		//setSelectionLocation(selectionX*scaleValue, selectionY*scaleValue);
		roiManager("Add");
		roiManager("Deselect");
	}

	for(i=0; i<nSelections; i++){ // delete originals
		roiManager("select", 0);
		roiManager("Delete");
		roiManager("Deselect");
	}
	...
	for(i=0; i<nSelections; i++){ // rename resized ones to original names and save xy bounding coords
		roiManager("select", i);
		roiManager("Rename", selNames[i]);
		roiManager("Deselect");
	}
	...
	setSlice(lumSlice);
	scaleString = "x=" + scaleValue + " y=" + scaleValue + " interpolation=Bilinear average create title=Scaled";
	run("Scale...", scaleString);
	roiManager("Show All with labels");
...
for(j=0; j<nLoops; j++){ // ROI loop
	...
	while(i <=endSize+0.001){
		...
		//	setResult("pattern_size", nSteps + resultsRow, i);
		//	setResult("Label", nSteps + resultsRow, rowLabel + "_" + selNames[j]);
		//}
	...
	//if(outputPatternSpectrum == 0) // set image label if it hasn't already been added above
	setResult("Label", resultsRow, rowLabel + "_" + selNames[j]);
	

	//for(i=0; i<lumBands; i++) // bands used for histogram
	//	setResult("Label", i + resultsRow, rowLabel + "_" + selNames[j]);
