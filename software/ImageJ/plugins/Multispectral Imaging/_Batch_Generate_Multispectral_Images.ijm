...
for (q = i; q < (images.length); q++){
	...
	configFilePath =  directory + imageName.substring(0, imageName.length - 4) + ".mspec";
	...
	if(renameRAWs ==1)
		for(i=0; i<photoNames.length; i++)
			File.rename(directory + photoNames[i], seqDirectory + imageName + photoNames[i]);
	...
	run("mica import");
	roiManager("save", directory + filename.substring(0, filename.length - 4) + ".zip")
	close("*");
}