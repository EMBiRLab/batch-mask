...
settingsFilePath = getDirectory("plugins") + "Multispectral Imaging/screeningSettings.txt";
...
for(i=0; i< rawList.length; i++){
	...
	if(verbose == 1){
		print("\\Clear");
		dcrawString = "open=[" + fileDir + rawList[i] + "] use_temporary_directory white_balance=None do_not_automatically_brighten output_colorspace=raw read_as=[16-bit linear] interpolation=[High-speed, low-quality bilinear]  do_not_rotate show_metadata" ;
	}else	
		dcrawString = "open=[" + fileDir + rawList[i] + "] use_temporary_directory white_balance=None do_not_automatically_brighten output_colorspace=raw read_as=[16-bit linear] interpolation=[High-speed, low-quality bilinear]  do_not_rotate" ;
	...
	if(i==0){
		...
		titlePath =  getDirectory("plugins") + "Multispectral Imaging/Photo Screening Page.png";
		open(titlePath);
