...
// DCRAW import linear:
path = "C:/Users/timre/Desktop/inference_set/"
file = File.openAsString(path + "dataset.csv")
images = split(file, "\n");
...
if(resume != ""){
	for (i = 0; i < (images.length); i++){
		filename = split(images[i], ",");
		if(filename[0] == resume) {
			break;
		}
	}
}
...
for (q = i; q < (images.length); q++){
	print(q + " out of " + images.length);
	...
	//  USER OPTIONS
	    ...
	    */
	    ...
	    settingsChoice = "Visible";
	    nonLin = true;
	    greyLocation = "Same Photo";
	    estimateBlack = false;
	    standardString = "5, 95";
	    customiseLevels = false;
	    customiseLocation = false;
	    imagesSequential = false;

	    //autoAlignOption = Dialog.getCheckbox();
	    autoAlignOption = "None";
	    autoAlignOffset = 16;
	    autoAlignLoops = 5;
	    autoAlignScaleStepSize = 0.005;
	    //autoAlignProportion = Dialog.getNumber();
	    autoAlignProportion = 1-(2*autoAlignScaleStepSize);

	    autoAlignCustomZone = false;

	    //saveConfig = Dialog.getCheckbox();
	    imageOutput = "Aligned Normalised 32-bit";
	    filename = split(images[q], ",");
	...
	for(j=0; j<nPhotos; j++){
		...
		if(imagesSequential == 1){
	        ...
	        // BEGIN MY CODE!!!
	        if(photoSettings[0] == "uv"){
	            imagePath="C:\\Users\\timre\\Desktop\\Snake Pattern\\DSC_1416.jpg";
	        }
	        else{

	        	if(File.exists(imageName + ".jpg") == 1){
	...
	configFilePath =  path + imageName + ".mspec";
	...
	if(renameRAWs ==1)
		for(i=0; i<photoNames.length; i++)
			File.rename(path + photoNames[i], seqDirectory + imageName + photoNames[i]);
	...
	run("mica import");
	roiManager("save", path + filename[0] + ".zip")
	close("*");
}